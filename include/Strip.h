...
class Strip {
   protected:
    ...
    //
    // The data for this strip, which is too big, but there you go.
    RGB* data;
    ...
    //
    // The color order for this strip. Default is GRB for WS2812B, per the data
    // sheet.
   ...
   public:
    ...
    /// @param colorIndex The index into the color map for the pixel.
    void putPixel(const RGB& color, uint p);

    /// @brief Gets the index of the color value at a given position in the
    /// strip
    /// @param p the position to get the color from
    /// @return the color at the given position.
    const RGB& get(uint p);
    ...
    /// @param colorIndex the index into the color map for the color to fill
    /// with
