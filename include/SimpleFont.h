...
class SimpleFont : public Font {
   public:
    ...
    SimpleFont(const uint8_t* fontData);
    ...
    uint render(Canvas* canvas, const char* text, int bx, int by,
                const RGB& color, bool renderMissing = true, RenderAngle angle = RENDER_0) override;
    ...
    /// @param text
    uint getWidth(const char* text, bool draw_missing = true) const override;

    std::pair<uint, uint> getBoundingBox(
        const char* text, bool draw_missing = true) const override {
        return std::pair<uint, uint>(getWidth(text, draw_missing), 0);
    }

    /// @brief Gets the width of the given character when rendered in this font.
    /// @param c the character
    /// @return the width of the character
    uint getWidth(char c, bool draw_missing = true) const;
